...
void tekstoAtskyrimas(string& tekstas, string line, double& skaicius) {
...
void getData(menuItemType menu[]) {
    string line;
    ifstream maistas("maistas.txt");
    int x = 0;
    while (getline(maistas, line)) {
...
void showMenu (menuItemType menu[]) {
    ...
    cout << "Pasirinkite maista paspaudus jo skaiciu" << endl;
    for (int i = 0; i < sizeof(menu); i++) {
        int tarpai = ilgasString(menu)-menu[i].menuItem.length();
        menu[i].menuItem.append(tarpai,'');
...
void spausdintiCeki(menuItemType menu[], int kiekiai[]) {
...
int main() {
    ...
    spausdintiCeki(menuList, kiekiai);
        return 0;
    }
